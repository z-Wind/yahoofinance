...
func TestPeriodCall_doRequest(t *testing.T) {
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{"Test", NewHistoryService(yfinanceTest).Period("0050.TW", "1mo", "1d"),
			"https://query1.finance.yahoo.com/v8/finance/chart/0050.TW?events=div%2Csplits&includeAdjustedClose=true&interval=1d&range=1mo", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rsp, err := tt.c.doRequest()
			if (err != nil) != tt.wantErr {
				t.Errorf("PeriodCall.doRequest() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			got := rsp.Request.URL.String()
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("PeriodCall.doRequest() = \n%v, want \n%v", got, tt.want)
			}
		})
	}
}
func TestPeriodCall_Do(t *testing.T) {
	str := `{
		"chart": {
		  "result": [
			{
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{"Test", NewHistoryService(yfinanceTest).Period("VTI", "max", "1d"), &Infomation{
...
func TestBetweenCall_doRequest(t *testing.T) {
	client := clientTest("", http.StatusOK)
	yfinanceTest, _ := New(client)
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{"Test", NewHistoryService(yfinanceTest).Between("0050.TW", time.Date(2009, time.November, 10, 23, 0, 0, 0, time.UTC), time.Date(2020, time.November, 12, 23, 0, 0, 0, time.UTC)),
			"https://query1.finance.yahoo.com/v8/finance/chart/0050.TW?events=div%2Csplits&includeAdjustedClose=true&interval=1d&period1=1257894000&period2=1605222000", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rsp, err := tt.c.doRequest()
			if (err != nil) != tt.wantErr {
				t.Errorf("BetweenCall.doRequest() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			got := rsp.Request.URL.String()
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("BetweenCall.doRequest() = \n%v, want \n%v", got, tt.want)
			}
		})
	}
}
func TestBetweenCall_Do(t *testing.T) {
	str := `{
		"chart": {
		  "result": [
			{
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{"Test", NewHistoryService(yfinanceTest).Between("VTI", time.Date(2009, time.November, 10, 23, 0, 0, 0, time.UTC), time.Date(2020, time.November, 12, 23, 0, 0, 0, time.UTC)), &Infomation{
			ServerResponse: ServerResponse{
				HTTPStatusCode: 200,
				Header:         map[string][]string{},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rsp, err := tt.c.Do()
			if (err != nil) != tt.wantErr {
				t.Errorf("BetweenCall.Do() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			got := rsp
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("BetweenCall.Do() = \n%+v, \nwant \n%+v", got, tt.want)
			}
		})
	}
}
